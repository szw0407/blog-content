...
变元赋值$u: {x_1, x_2, \cdots} \to D_I$。就是给里面每个变元指派一个元素作为其解释。对于一个解释，可以有多个赋值。这个很显然。如果有两个赋值满足除了$x_i$外的一切变元赋值都相同，那么这两个赋值是i-等值的。

### 满足

$I$的一个赋值$u$满足$A$，记作$I, u \models A$，或者$I, v \models A\[u\]$，其中$u$是对应的变元赋值。简写成$I \models A$，其就是满足所有的原子公式为真。